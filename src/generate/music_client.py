...
async def _generate_track_async(
    ...
    try:
        ...
        logger.warning("Lyria track '%s' failed: %s", track_name, e)
...
def build_full_prompt_dict(
    ...
    # Start with LLM-generated prompts (combat + maze_* tracks)
    for name, prompt in llm_prompts.items():
        if prompt:
            merged[name] = prompt

    # Fill in fixed tracks (or override LLM null with fixed)
